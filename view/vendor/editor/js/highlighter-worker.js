const rex_html = /(?<comment>&lt;!--[\s\S]*?--&gt;)|(?<tag>&lt;\/?[a-z0-9:\-]+[\s\S]*?&gt;)/gi;
const rex_code =
    /(?<comment>\/\/.*|\/\*[\s\S]*?\*\/)|(?<string>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')|(?<keyword>\b(?:function|return|var|let|const|if|else|for|while|php|echo|public|class|static|this|import|void|private|package|protected|new|extends|super|true|false|export|final|null|implements|interface|async|try|catch|switch|case|finally|break)\b)|(?<number>\b\d+\b)|(?<keyVar>\b(?:int|string|boolean|float|Toast|Button|TextView|LinearLayout|View|Bundle|double|char|long|short|byte|ArrayList|Context|Intent|Bitmap|HashMap|Override)\b)/gi;
const escapeHTML = (str) => str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/ $/gm, ' \u00A0');
...
self.onmessage = function (e) {
    ...
    let highlighted = codes.replace(combinedRegex, (match, ...args) => {
        ...
        if (isXML && groups.tag) {
            return match.replace(/(&lt;\/?[a-z0-9:\-]+)([\s\S]*?)(&gt;)/gi, (m, t, b, e) => {
                const attrs = b.replace(/([\w\-\:]+)=((["\']).*?\3)/g, '<sp class="a">$1</sp>=<sp class="s">$2</sp>');
