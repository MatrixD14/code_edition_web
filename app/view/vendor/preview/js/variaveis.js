...
const shapeHandlers = {
  ...
  corners(el, node) {
    const radius =
      node.getAttribute("android:radius") ||
      node.getAttribute("android:topLeftRadius");

    if (radius) el.style.borderRadius = toPx(radius);
  },

  padding(el, node) {
    if (node.getAttribute("android:top"))
      el.style.paddingTop = toPx(node.getAttribute("android:top"));

    if (node.getAttribute("android:bottom"))
      el.style.paddingBottom = toPx(node.getAttribute("android:bottom"));

    if (node.getAttribute("android:left"))
      el.style.paddingLeft = toPx(node.getAttribute("android:left"));

    if (node.getAttribute("android:right"))
      el.style.paddingRight = toPx(node.getAttribute("android:right"));
  },
};
const viewHandlers = {
  LinearLayout(node) {
